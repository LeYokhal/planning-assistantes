...
> **Version 1.6 — 07/09/2026.** Complète `PLANNING_ASSISTANTES_BRIEF.md` (référence du contrat MCP Doctolib, § 3.2 et § 9.6) et `WORKING_HABITS.md` (régime de travail, applicable tel quel).
> Statut : **périmètre et décisions produit figés** (C2.1 → C2.12, C3.1 → C3.8, C4.1 → C4.13). **Briques 1a, 1b, 2, 3, 4a et 4b livrées** (§ 8, § 14) : le périmètre v1 de l'app est complet, reste la brique 5 (n8n) et la brique 0 (VoiceDoctolib). Restent un point de rétention (§ 10, C2.6) et l'étalon de paie (§ 11).
...
- **Aucune donnée RH sensible hors besoin** : pas de n° de sécurité sociale, date de naissance, téléphone personnel, IBAN, adresse — ces champs restent là où ils sont (Notion, Google Sheets). **Le type d'une absence et sa précision ne sortent jamais de l'application** : ni dans l'audit, ni dans les logs, ni dans un webhook, ni dans les données de paie (§ 5.2). **Amendement C4.1 (06/09/2026)** : le libellé du type figure dans le `DATA` de la page planning, donc à l'écran (rôles `cabinet` et `principale`) **et dans la copie HTML autonome téléchargée** ; il ne figure ni dans `STATE`, ni dans les versions, ni dans l'export JSON.
...
| **C3.8** | **Existant Notion des absences** | **aucune migration** : l'existant est ressaisi à la main dans l'admin par `cabinet` ; Notion devient archive en lecture seule ; à partir de là, les absences entrent par les salariées et les décisions | décision du 01/09/2026 ; le critère « un mois migré à l'identique » du § 8 est remplacé |
...
4. **Paie — côté app en place (3)** : `GET /api/n8n/paie/<AAAA-MM>/` rend, par salariée ayant au moins une absence effective d'un type `paie` **dont une portion tombe dans le mois calendaire**, la portion du mois, le total de l'absence, les drapeaux `facture_partiellement` / `corrigee` / `repartition_calculee`, et le paragraphe mis en forme (« Paie 2026-09 — absences à décompter : NOM Prénom : 2,5 jour(s). », avec une phrase d'alerte si une répartition n'a pu être calculée). Une salariée sans absence comptée n'apparaît pas. **Le mail à la comptable reste la brique 5.**
...
- **Sorties hors de l'app** — exactement une porte les noms : l'endpoint de paie, réservé à n8n, qui transmet **nom, nombre de jours et dates, jamais le type ni la précision**. Les webhooks d'absence portent identifiants, dates, statut, lien — rien d'autre. Vérifié en recette (§ 14).
...
- Admin Django : `TypeAbsence` modifiable ; `AbsenceSalariee` en création, modification et suppression journalisée (canal de reprise de l'existant Notion et de nettoyage) ; les absences ne portent que des personnes assistante / secrétaire (`clean_personne` rend une erreur de champ lisible ; `save_model` est le dernier rempart).
...
| 5 | Notifications | n8n | mail comptable depuis `GET /api/n8n/paie/`, **comparaison à l'étalon (S3b) avant bascule**, tir de test vers Yohan ; mails du backlog si souhaité | même contenu qu'aujourd'hui ; ancien workflow désactivé, pas supprimé | à faire |
...
- **Étalon de paie** : la sortie du workflow « PROD – Fiche de paie V2 » sur un mois réel, **sans l'adresse de la comptable**, pour comparer à `GET /api/n8n/paie/` une fois l'existant ressaisi — critère S3b différé de la recette 3, **bloquant avant la brique 5**.
